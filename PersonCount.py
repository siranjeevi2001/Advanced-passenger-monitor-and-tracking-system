...
class SocialDistanceAnalysis:
    ...
    def SDAProcess(self, image):
        ...
        '''ff=open("safe.txt","w")
        ff.write(str(safePeople))
        ff.close()

        ff=open("unsafe.txt","w")
        ff.write(str(riskPeople))
        ff.close()'''
...
class VideoCamera2(object):
    def __init__(self):
        ...
        self.filename = "videos/video2.mp4"

        self.opname = "outputs/output_" + self.filename.split('/')[1][:-4] + '.avi'

        self.assetpath = "assets/"
        self.yolopath = "yolo-coco/"
        #filename
        self.video = cv2.VideoCapture(0)
        self.fno = 0
        self.writer = None

        self.AnalysisObj = SocialDistanceAnalysis()

        #self.fulltick = time.time()
    ...
    def get_frame(self):
        success, outputFrame = self.video.read()

        
        #if not success:
        #    break
        currentImg = outputFrame.copy()
        ...
        if(self.fno%2 == 0 or self.fno == 1):
            self.AnalysisObj.SDASetup(self.yolopath, self.assetpath, videoShape)
            self.AnalysisObj.SDAProcess(currentImg)
            outputFrame = self.AnalysisObj.SDADisplay()

            #cv2.imshow('SocialDistanceDashboard', outputFrame)

            '''if self.writer is None:
                fourcc = cv2.VideoWriter_fourcc(*"MJPG")
                self.writer = cv2.VideoWriter(self.opname, fourcc, 30,
                    (outputFrame.shape[1], outputFrame.shape[0]), True)'''

        #self.writer.write(outputFrame)
        #self.fulltock = time.time()
